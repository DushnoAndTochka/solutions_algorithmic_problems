...
class Solution(object):
    ...
    (M - 2 * Layer) * (N - 2 * Layer) - кол-во чисел в конкретном слое. так как каждый внутренний квадрат
    становится меньше в высоту и в ширину на 2 числа.
    Выходит, что если мы из общей кол-ва чисел вычтем кол-во чисел текущего слоя, 
    то получим как раз самое большое число, которое уже заполнили.
    ...
    def spiral_matrix(self, n: int, m: int) -> List[List[int]]:
        ...
        def filling_layer(layer=0):
            ...
            for i in range(column_count-1):
                res[layer + row_count - 1][layer + column_count - 1 - i] = current_num
                current_num += 1
